...
function productCard(p) {
  ...
  const wa  = cleanWaNumber(p.wa || settings.defaultWa);
  const msg = `Hello! 👋\n\nI'm interested in purchasing:\n*${p.name}*\nColor: ${p.color}\nPrice: ${inr(price)}\n\nPlease share availability and details. Thank you!`;
  const encodedMsg = encodeURIComponent(msg).replace(/'/g, '%27');
  ...
  <div class="product-card">
    ...
    <div class="product-body">
      ...
      <button class="btn-order" onclick="orderWa('${wa}','${encodedMsg}')">
...
function orderWa(num, encodedMsg) {
  const phone = cleanWaNumber(num || settings.defaultWa);
  if (!phone) {
    alert('WhatsApp number is not set yet.');
    return;
  }
